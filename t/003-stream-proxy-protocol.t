...
--- stream_config
    upstream backend {
        server 127.0.0.1:12345 max_fails=3 fail_timeout=5s;
    }
...
--- http_config
    server {
        listen 127.0.0.1:12345 proxy_protocol;
...
--- stream_config
    upstream backend {
        server [::1]:12345 max_fails=3 fail_timeout=5s;
    }
--- stream_server_config
    proxy_pass backend;
    set_real_ip_from  127.0.0.1/32;
